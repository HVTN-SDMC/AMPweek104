...
ggplotSievePH_wk80wk104 <- function(fit.te, data1, data2, limits.x=NULL, breaks.x=NULL, labels.x=breaks.x, 
  ...
  for (i in 1:length(fit.te)){
    fit.te[[i]]$te$cohort <- factor(fit.te[[i]]$cohort, levels=c( "Week 80", "Week 104"))
    
    p1 <- p1 + 
      geom_ribbon(aes(x=mark, ymin=LB, ymax=UB, fill=cohort), data=fit.te[[i]]$te, alpha=0.2) +
      geom_line(aes(x=mark, y=TE, colour=cohort, linetype = cohort), data=fit.te[[i]]$te, na.rm=TRUE, size=1.9)
  }
  
  
  #browser()
  legendLabels <- c( "Weeks 0 to 80", "Weeks 0 to 104")
  vjustbottom <- ifelse(grepl("\n",xlab), -5, -0.5)
  legendpos <- ifelse(grepl("\n", xlab), -0.3, -0.25)
  breaks.y <- round(seq(-0.6, 1, 0.2), 1)
  p1 <- p1 +
    scale_colour_manual(values=c("blue4", "red4"), name="", labels=legendLabels, guide=guide_legend(ncol=2, title.position="left")) +
    scale_fill_manual(values=c("blue4", "red4"), name="", labels=legendLabels, guide=guide_legend(ncol=2, title.position="left")) +
    scale_linetype_manual(values=c("solid", "11"), name="", labels=legendLabels, guide=guide_legend(ncol=2, title.position="left"))+
  ...
  data <- rbind(data1, data2) %>% 
    mutate(cohort=factor(c(rep("Week 80", length(data1$tx)), 
                           rep("Week 104", length(data2$tx))), levels=c("Week 80", "Week 104")))
  
  data$txchort <- factor(interaction(data$treatment, data$cohort), 
                         levels=c(  "Placebo.Week 104", "VRC01.Week 104", "Placebo.Week 80","VRC01.Week 80"))
  set.seed(1)
  p2 <- ggplot(data) +
    ggtitle(title) +
    geom_boxplot(aes(x=txchort, y=mark, colour=cohort), outlier.shape=NA, na.rm=TRUE) +
    geom_jitter(aes(x=txchort, y=mark, colour=cohort), na.rm=TRUE, width=0.2, height=0, shape=21, fill="white", 
                size=2, stroke=1.3) +
    scale_colour_manual(values=c( "blue4", "red4"), guide="none") +
    coord_flip() +
    scale_x_discrete(name=NULL, labels=c("VRC01.Week 80"="VRC01", "Placebo.Week 80"="Placebo", 
                                         "VRC01.Week 104"="VRC01", "Placebo.Week 104"="Placebo")) +
...
ggplotSievePH_onecohort <- function(fit.te, data, limits.x=NULL, breaks.x=NULL, labels.x=breaks.x, 
                                    xlab, limits.y=NULL, sieveTestLabel=NULL, title=NULL, color_line = "orange", 
                                    height.panels = c(1, 2), cohort = "Week 104", legendLabels = "Weeks 0 to 104" ){
  if (is.null(limits.x)){
    limits.x <- range(c(data$mark), na.rm=TRUE)
  }
  ...
  p1 <- p1 + 
    geom_ribbon(aes(x=mark, ymin=LB, ymax=UB, fill = cohort), data=fit.te$te, alpha=0.2) +
    geom_line(aes(x=mark, y=TE, color = cohort), data=fit.te$te, na.rm=TRUE, size=1.9)+
    scale_colour_manual(values=color_line, name="", labels=legendLabels, guide=guide_legend(ncol=1, title.position="left")) +
    scale_fill_manual(values=color_line, name="", labels=legendLabels, guide=guide_legend(ncol=1, title.position="left")) 
  ...
  p2 <- ggplot(data) +
    ggtitle(title) +
    geom_boxplot(aes(x=treatment, y=mark), outlier.shape=NA, na.rm=TRUE,  color = color_line) +
    geom_jitter(aes(x=treatment, y=mark), na.rm=TRUE, width=0.35, height=0, shape=21, fill="white", 
                size=2, stroke=1.3, color = color_line) +
    coord_flip() +
    scale_x_discrete(name=NULL, labels=c("VRC01"="VRC01", "Placebo"="Placebo")) +
    theme_bw() +
    theme(plot.title=element_text(hjust=0.5, size=15),
          axis.text.y=element_text(size=12, colour="black"),
          axis.ticks.x=element_blank(),
  ...
  p <- p2 + p1 + plot_layout(ncol=1, heights= height.panels )
