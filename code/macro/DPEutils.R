...
forestplot.logisticReg<- function(result, xlog = FALSE, ref_line = 0, ticks_digits, xlim.x, ticks_at.x, ci_cols,
  ...
  ggplot2::ggsave(filename = outputFileName, 
                  ...
                  path = outputDir,
                  dpi = 320,
                  width = width, height = 0.25*(length(dt$mean))+4,units = "in")  
}
...
result.summary <- function(fit.list, dose, .x, master.trial){
 # browser()
  if(dose == "T1+T2"){
  ...
  .inc.1.trt <- .n.events.type.1.trt/(sum(master.trial$hiv1fpday[master.trial$txLabel == 1], na.rm = TRUE)/365.25)*100
  .inc.0.trt <- .n.events.type.0.trt/(sum(master.trial$hiv1fpday[master.trial$txLabel == 1], na.rm = TRUE)/365.25)*100
  .inc.1.placebo <- .n.events.type.1.placebo/(sum(master.trial$hiv1fpday[master.trial$txLabel == 0], na.rm = TRUE)/365.25)*100
  .inc.0.placebo <- .n.events.type.0.placebo/(sum(master.trial$hiv1fpday[master.trial$txLabel == 0], na.rm = TRUE)/365.25)*100 
