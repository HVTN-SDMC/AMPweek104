...
confidenceband.loghazardratio <- function(time,delta,Rx,u1,u2,t1,t2,N=1000,band11=0,
  ...
  answermat <- cbind(timesunique,log(ee2/ee1),lowint,upint,lowband95,upband95,
                     lowband90,upband90,lowband80,upband80,ee1,ee2,vee1,vee2, band111, band112, band1opt, band2opt)
...
plotSmoothHazTE <- function(data,  summaryType = "efficacy", 
                             title, xlab, ylab, xtickAt, xtickLabel, xMax=NULL, showLegend=TRUE, 
                            phTestPvalue = NULL,
                             nBoot=NULL, loadFile=NULL, saveFile=NULL, saveDir=NULL, band11 = NULL){

  # standardize variable names
  data$tx <- data$treatment #must be 1-placebo and 2-treatment
  data$fTime <- data$time
  data$fInd <- data$status
  
  # set input parameters
  N <- 10000
  
  # set u1=t1 to the maximum of the smallest treatment-specific failure times
  t1 <- max(tapply(data$fTime, data$tx, cgwtools::minn, nth = 1))
  
  # set u2=t2 to the minimum of the 2nd largest treatment-specific failure times
  t2 <- min(tapply(data$fTime, data$tx, cgwtools::maxn, nth = 2))
  ...
  par(mar=c(5, 6.5, 4, 2), las=1, cex.axis=1.2) # bltr
  cexPlotTitle <- 1.4
  cexAxisLabel <- 1.3
  cexLegend <- 1.2
  
  if(summaryType == "efficacy"){
    tranform.f <- function(x){1-exp(x)}
    yLim <- c(-1.2, 1.2)
    
  }else if (summaryType == "hazardRatio"){
    tranform.f <- function(x){
      y = x
      return(x)
    }
    yLim <- c(log(0.08), log(4.3))
  }
  
  plot(timeGrid, tranform.f(logHR), xlab="", ylab="", type="n", xaxt="n", yaxt="n", xlim=c(0, xMax), 
       ylim=yLim, bty="l")
  
  #axis(1, at=seq(0, xMax, by=30.5), labels=seq(0, xMax, by=30.5)/30.5)
  if(summaryType == "efficacy"){
    axis(2, at=seq(-1, 1, by=0.25), labels=seq(-100, 100, by=25))
  }else if(summaryType == "hazardRatio"){
    axis(2, at=log(c(0.1,0.25, 0.5, 0.75, 1, 1.33, 2, 4)), label=c(0.1, 0.25, 0.5, 0.75, 1, 1.33, 2, 4))
  }
  
  
  axis(1, at=xtickAt, labels=xtickLabel)
  axis(1, at=108, labels= 108)
  mtext(xlab, side=1, line=2.8, cex=cexAxisLabel)
  abline(h=0, col="gray50", lwd=2)
  mtext(ylab, side=2, line=3.3, cex=cexAxisLabel, las=0)
  mtext(title, side=3, line=0.5, cex=cexPlotTitle, las=0)
  legend(10, yLim[2]+0.15, legend=paste("PH Test P-value", phTestPvalue), cex=0.9*cexLegend, bty="n")
  
  
  lines(timeGrid, tranform.f(logHR), lwd=3.5)
  lines(timeGrid, tranform.f(ptLB), lty="dashed", lwd=3)
  lines(timeGrid, tranform.f(ptUB), lty="dashed", lwd=3)
  lines(timeGrid, tranform.f(smLB), lty="dotted", lwd=3)
  lines(timeGrid, tranform.f(smUB), lty="dotted", lwd=3)
  
  return(logHRfit[, 15:18])
 
}

plotSmoothHazTE_wk80_postWk80 <- function(dataWk80, dataPostWk80,  summaryType = "efficacy", 
                            title, xlab, ylab, xtickAt, xtickLabel, xMax=NULL, 
                            nBoot=NULL, band11 = NULL){
  
  # standardize variable names
  dataWk80$tx <- dataWk80$treatment #must be 1-placebo and 2-treatment
  dataWk80$fTime <- dataWk80$time
  dataWk80$fInd <- dataWk80$status
  dataPostWk80$tx <- dataPostWk80$treatment #must be 1-placebo and 2-treatment
  dataPostWk80$fTime <- dataPostWk80$time
  dataPostWk80$fInd <- dataPostWk80$status
  
  # set input parameters
  N <- 10000
  
  # set u1=t1 to the maximum of the smallest treatment-specific failure times
  t1 <- max(tapply(dataWk80$fTime, dataWk80$tx, cgwtools::minn, nth = 1))
  
  # set u2=t2 to the minimum of the 2nd largest treatment-specific failure times
  t2 <- min(tapply(dataWk80$fTime, dataWk80$tx, cgwtools::maxn, nth = 2))
  ...
  logHRfitWk80<- confidenceband.loghazardratio(dataWk80$fTime, dataWk80$fInd, dataWk80$tx, t1, t2, t1, t2, N, band11, band1, band12, band2, ngrid, biasadjust, tailsl, tailsu, nBoot, Nv)  
 
  timeGridWk80 <- logHRfitWk80[, 1]
  if (is.null(xMax)){ xMax <- max(timeGridWk80) }
  
  logHRWk80 <- logHRfitWk80[timeGridWk80<=xMax, 2]
  ptLBWk80 <- logHRfitWk80[timeGridWk80<=xMax, 3]
  ptUBWk80 <- logHRfitWk80[timeGridWk80<=xMax, 4]
  smLBWk80 <- logHRfitWk80[timeGridWk80<=xMax, 5]
  smUBWk80 <- logHRfitWk80[timeGridWk80<=xMax, 6]
  timeGridWk80 <- timeGridWk80[timeGridWk80<=xMax]
 # browser()
  
  # set u1=t1 to the maximum of the smallest treatment-specific failure times
  t1 <- max(tapply(dataPostWk80$fTime, dataPostWk80$tx, cgwtools::minn, nth = 1))
  
  # set u2=t2 to the minimum of the 2nd largest treatment-specific failure times
  t2 <- min(tapply(dataPostWk80$fTime, dataPostWk80$tx, cgwtools::maxn, nth = 2))
  ...
  logHRfitPostWk80<- confidenceband.loghazardratio(dataPostWk80$fTime, dataPostWk80$fInd, dataPostWk80$tx, t1, t2, t1, t2, N, band11, band1, band12, band2, ngrid, biasadjust, tailsl, tailsu, nBoot, Nv)  
  
  timeGridPostWk80 <- logHRfitPostWk80[, 1]
  if (is.null(xMax)){ xMax <- max(timeGridPostWk80) }
  
  logHRPostWk80 <- logHRfitPostWk80[timeGridPostWk80<=xMax, 2]
  ptLBPostWk80 <- logHRfitPostWk80[timeGridPostWk80<=xMax, 3]
  ptUBPostWk80 <- logHRfitPostWk80[timeGridPostWk80<=xMax, 4]
  smLBPostWk80 <- logHRfitPostWk80[timeGridPostWk80<=xMax, 5]
  smUBPostWk80 <- logHRfitPostWk80[timeGridPostWk80<=xMax, 6]
  timeGridPostWk80 <- timeGridPostWk80[timeGridPostWk80<=xMax]
  
  logHR <- c(logHRWk80, logHRPostWk80)
  ptLB <- c(ptLBWk80, ptLBPostWk80)
  ptUB <- c(ptUBWk80, ptUBPostWk80)
  smLB <- c(smLBWk80, smLBPostWk80)
  smUB <- c(smUBWk80, smUBPostWk80)
  timeGrid <- c(timeGridWk80, timeGridPostWk80 + 80)
